...
def season_segment_hourly(
    df_hour: pd.DataFrame,
    # day-aggregation columns
    time_col: str = "time",
    temp_col: str = "temperature_2m",
    # season params (DAY SCALE)
    T0: float = 0.0,
    g_days: int = 6,
    winter_min_len_days: int = 7,
    nonwinter_min_seg_len_days: int = 15,
    # winter bridge params (keep your new logic)
    post_cold_min_days: int = 5,
    write_cold: bool = True,
) -> Dict[str, Dict]:
    """
    小时数据 -> 日聚合 -> 日尺度切分 -> 映射回小时索引
    返回结构兼容 full_experiment_pipeline.py 当前用法：
      {
        "winter": { "W_idx": [(l_hour,r_hour), ...], "W_date": [(dt_l,dt_r), ...], ... },
        "segmentation": { "seasons": [{"spring":(...), "summer":(...), "autumn":(...)}, ...], ... }
      }
    """
    # --------
    # 0) clean hourly df
    ...
    df_hour = df_hour.dropna(subset=[time_col]).sort_values(time_col).reset_index(drop=True)

    # --------
    # 1) hourly -> daily aggregation (same logic as read_data(), but from df not csv)
    # --------
    dfh = df_hour.set_index(time_col)

    agg = {
        temp_col: ["min", "mean"],
    }
    # 兼容 rain/snow列是否存在
    if "rain" in dfh.columns:
        agg["rain"] = "sum"
    if "snowfall" in dfh.columns:
        agg["snowfall"] = "sum"

    dfd = dfh.resample("D").agg(agg)

    dfd.columns = [
        "temperature_2m_min" if c == (temp_col, "min") else
        "temperature_2m_mean" if c == (temp_col, "mean") else
        "rain_sum" if c == ("rain", "sum") else
        "snowfall_sum" if c == ("snowfall", "sum") else
        f"{c[0]}_{c[1]}"
        for c in dfd.columns
    ]
    dfd = dfd.reset_index().rename(columns={time_col: "time"})
    dfd = dfd.dropna(subset=["temperature_2m_min", "temperature_2m_mean"]).reset_index(drop=True)

    # --------
    # 2) run day-scale winter + segmentation (your new logic)
    # --------
    w = winter(
        df=dfd,
        T0=T0,
        ...
        post_cold_min=post_cold_min_days,
        date_col="time",
    ...
    seg = segmentation(
        ...
        min_seg_len=nonwinter_min_seg_len_days,
        date_col="time",
    )

    # --------
    # 3) map day index ranges -> hourly index ranges
    # --------
    dates_day = w["df"]["time"].tolist()

    def map_ranges(day_ranges):
        out = []
        for l_day, r_day in day_ranges:
            day_start = dates_day[l_day]
            day_end = dates_day[r_day]
            l_hour, r_hour = day_seg_to_hour_seg(
                df_hour=df_hour,
                day_start=day_start,
                day_end=day_end,
                time_col=time_col,
    ...
    # winter
    W_hour = map_ranges(w["W_idx"])
    W_date_hour = [(df_hour.loc[l, time_col], df_hour.loc[r, time_col]) for (l, r) in W_hour]

    winter_out = dict(w)
    winter_out["W_idx_day"] = w["W_idx"]
    winter_out["W_date_day"] = w["W_date"]
    winter_out["W_idx"] = W_hour
    winter_out["W_date"] = W_date_hour

    # segmentation: seasons list + spring/summer/autumn idx lists
    seg_out = dict(seg)
    seg_out["NW_idx_day"] = seg["NW_idx"]
    seg_out["NW_idx"] = map_ranges(seg["NW_idx"])

    # seasons (flatten each dict’s ranges)
    seasons_hour = []
    for s in seg["seasons"]:
        out_s = {}
        for name in ("spring", "summer", "autumn"):
            if s.get(name) is None:
                out_s[name] = None
            else:
                out_s[name] = map_ranges([s[name]])[0]
        seasons_hour.append(out_s)
    seg_out["seasons_day"] = seg["seasons"]
    seg_out["seasons"] = seasons_hour

    # idx lists
    seg_out["spring_idx_day"] = seg["spring_idx"]
    seg_out["summer_idx_day"] = seg["summer_idx"]
    seg_out["autumn_idx_day"] = seg["autumn_idx"]
    seg_out["spring_idx"] = map_ranges(seg["spring_idx"]) if seg["spring_idx"] else []
    seg_out["summer_idx"] = map_ranges(seg["summer_idx"]) if seg["summer_idx"] else []
    seg_out["autumn_idx"] = map_ranges(seg["autumn_idx"]) if seg["autumn_idx"] else []

    # dates for hour idx lists
    seg_out["spring_date"] = [(df_hour.loc[l, time_col], df_hour.loc[r, time_col]) for (l, r) in seg_out["spring_idx"]]
    seg_out["summer_date"] = [(df_hour.loc[l, time_col], df_hour.loc[r, time_col]) for (l, r) in seg_out["summer_idx"]]
    seg_out["autumn_date"] = [(df_hour.loc[l, time_col], df_hour.loc[r, time_col]) for (l, r) in seg_out["autumn_idx"]]
    seg_out["NW_date"] = [(df_hour.loc[l, time_col], df_hour.loc[r, time_col]) for (l, r) in seg_out["NW_idx"]]

    return {"winter": winter_out, "segmentation": seg_out}


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Winter segmentation for Dunhuang dataset")
    parser.add_argument("--path", type=str, default="back_con\data\processed_weather_data_bazhong.csv", help="path to csv")
