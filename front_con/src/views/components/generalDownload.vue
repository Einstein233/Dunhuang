...
export default {
  data() {
    return {
      ...
      fieldMap: {
        ...
        rain_sum: "降雨量",
        snow_sum: "降雪量",
        max_continuous_wind_speed: "最大持续风速",
        windgusts_max: "最大阵风",
        winddirection_dominant: "主导风向",
        shortwave_radiation_sum: "太阳辐照"
      },
    ...
    handleRegionChange(val) {
      if (val.includes("ALL_REGIONS")) {
        if (this.selectedRegions.length < this.regions.length) {
          this.selectedRegions = this.regions.slice();  // 全选所有地区
        } else {
          this.selectedRegions = [];  // 清空所有地区
        }
      }
    },
    ...
    async downloadData() {
      ...
      try {
        for (let region of this.selectedRegions) {
          const res = await axios.post('http://localhost:3000/weather/generalDownload', {
            fields: this.selectedFields,
            start: this.dateRange[0],
            end: this.dateRange[1],
            region: region  // 传递地区作为参数
          }, {
            responseType: 'blob'
          });
          ...
          a.href = url;
          a.download = `${region}_${this.dateRange[0]}_to_${this.dateRange[1]}.xlsx`; // 根据地区和日期命名文件
  ...
  watch: {
    selectedRegions(newVal, oldVal) {
      // 检查是否有选中项，如果没有选项，禁用清空按钮
